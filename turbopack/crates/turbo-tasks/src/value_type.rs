...
struct ValueTypeTraits {
    /// Trait type IDs that this value type implements, stored as raw u16.
    /// Used for lookup: linear scan to find the index, then index into `vtables`.
    trait_ids: SmallVec<[u16; 8]>,
    /// Vtables corresponding 1:1 with `trait_ids`.
    vtables: Vec<&'static [&'static NativeFunction]>,
}
...
impl ValueType {
    ...
    pub fn get_trait_method(
        ...
        let idx = info.trait_ids.iter().position(|&id| id == trait_type_id)?;
        Some(info.vtables[idx][trait_method.index as usize])
    }
    ...
    fn register_trait(
        ...
        traits.trait_ids.push(trait_type_id);
        traits.vtables.push(trait_methods);
    }

    #[inline]
    pub fn has_trait(&self, trait_type: &TraitTypeId) -> bool {
        let raw = **trait_type;
        self.trait_info().trait_ids.contains(&raw)
    }
}
